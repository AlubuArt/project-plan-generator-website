...
// Project plan generation schemas
...
// Template prompt function (simplified from the API route)
const getTemplatePrompt = (template = 'next') => {
...
function validateProjectIdeaInput(idea) {
...
// URL encoding function (simplified from utils)
function encodeForUrl(plan, idea) {
    try {
...
const server = new Server({
    ...
    version: '1.0.0',
    description: 'Generate comprehensive project plans from ideas using AI',
}, {
    capabilities: {
        tools: {},
        resources: {},
        ...
        tools: [
            {
                ...
                description: 'Generate a comprehensive project plan from a project idea using AI. Returns a structured plan with user stories, technical implementation details, and actionable tasks.',
                inputSchema: {
                    type: 'object',
                    properties: {
                        idea: {
                            type: 'string',
                            description: 'The project idea to transform into a plan (20-1000 characters)',
                            minLength: 20,
                            maxLength: 1000,
                        },
                        template: {
                            type: 'string',
                            enum: ['next', 'vercel-ai'],
                            description: "The template to use: 'next' for general Next.js projects, 'vercel-ai' for AI-powered projects",
                            default: 'next',
                        },
                    },
                    required: ['idea'],
            ...
            },
            {
                ...
                description: 'Validate a project idea for content and length requirements before generating a plan.',
                inputSchema: {
                    type: 'object',
                    properties: {
                        idea: {
            ...
            },
            {
                ...
                description: 'Encode a project plan and idea into a shareable URL-safe format.',
                inputSchema: {
                    type: 'object',
                    properties: {
                        plan: {
        ...
        resources: [
            {
                ...
                description: 'Information about the Next.js project template with App Router and TypeScript',
            },
            {
                uri: 'project-plan://templates/vercel-ai',
                name: 'Vercel AI Project Template',
                description: 'Information about the Vercel AI template with OpenAI integration',
            },
            {
                uri: 'project-plan://guidelines/user-stories',
                name: 'User Story Guidelines',
                description: 'Best practices for writing effective user stories with acceptance criteria',
            },
            {
                uri: 'project-plan://guidelines/technical-tasks',
                name: 'Technical Task Guidelines',
                description: 'Guidelines for breaking down technical implementation into actionable tasks',
            },
        ],
    };
});
    ...
    try {
        switch (name) {
            case 'generate_project_plan': {
                ...
                console.error(`Generating plan for idea: "${idea.substring(0, 50)}..." with template: ${template}`);
                ...
                console.error(`Successfully generated plan (${plan.length} characters)`);
                return {
                    content: [
                        {
                            type: 'text',
            ...
            case 'encode_plan_for_sharing': {
                ...
                const shareUrl = `${process.env.NEXT_PUBLIC_BASE_URL || 'http://localhost:3000'}/plan/${encodedData}`;
                return {
                    content: [
                        {
                            type: 'text',
                            text: JSON.stringify({
                                ...
                                expiresAt: null, // Plans don't expire
...
async function runServer() {
    ...
    console.error('Project Plan Generator MCP Server running on stdio');
}
