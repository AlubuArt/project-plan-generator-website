...
class ProjectPlanMCPClient {
  ...
  async encodeForSharing(plan: string, idea: string) {
    ...
    if (result.success) {
      ...
      console.log(
        `📦 Encoded data length: ${result.encodedData.length} characters`
      );
  ...
  async runDemo() {
    try {
      ...
      const testIdea =
        'Create a task management app for small teams with real-time collaboration, file sharing, and project timeline tracking. Users should be able to create projects, assign tasks, set deadlines, and communicate through comments.';
