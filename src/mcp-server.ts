...
type GeneratePlanArgs = z.infer<typeof GeneratePlanArgsSchema>;
type ValidateIdeaArgs = z.infer<typeof ValidateIdeaArgsSchema>;
type EncodeShareUrlArgs = z.infer<typeof EncodeShareUrlArgsSchema>;
    ...
    try {
      switch (name) {
        case 'generate_project_plan': {
          ...
          console.error(
            `Generating plan for idea: "${idea.substring(0, 50)}..." with template: ${template}`
          );
          ...
          console.error(
            `Successfully generated plan (${plan.length} characters)`
          );
